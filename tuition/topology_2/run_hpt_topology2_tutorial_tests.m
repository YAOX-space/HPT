...
expectedRoot = ["DCLink", "DQControl", "ElectromagneticTransformer", ...
    "CouplingTransformer", "EnergyConverter", "Grid", "Load", "MeasLV", "MeasMV", ...
...
ok = ok && rootOk;
checks(end + 1) = sprintf('root has separated 14 teaching blocks=%d', rootOk);

requiredRoot = ["ElectromagneticTransformer", "CouplingTransformer", ...
    "SeriesTransformer", ...
...
for k = 1:3
    ...
    pctPresent = ~isempty(find_system([model '/CouplingTransformer'], ...
        'LookUnderMasks', 'all', 'Name', pctName));
    tpfPresent = ~isempty(find_system([model '/CouplingTransformer'], ...
...
details = ['Checks W1/W2 main transformer, secondary-side parallel energy ' ...
    'path, separated W5/W6 series injection, shared DC-link, and absence of topology1 W3 logic.'];
end
